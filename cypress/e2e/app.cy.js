describe('Testes no site Livraria Cultura', () => {
  ...
  function abrirLogin() {
    ...
    cy.get('.col-lg-3 > .icon-user-wrapper > .login-hover > .login-inner > :nth-child(1) > .button > .bg-opacity', { timeout: 15000 })
      .should('be.visible')
      .click({ force: true })
  ...
  function botaoConfirmarOuEntrar() {
    return cy.get('#classicLoginBtn, button[type="submit"]', { timeout: 20000 })
      .filter(':visible')
      .first()
  }
  ...
  describe('Login VTEX', () => {
    ...
    it('Teste 6 — Acessar página de login ao clicar em Entrar', () => {
      ...
      // Alguns fluxos mostram primeiro “Escolha como entrar”.
      // Se aparecer o input de email direto, já valida. Senão, clica em “Entrar com email e senha”.
    ...
    it('Teste 7 — Login com credenciais inválidas exibe erro', () => {
      ...
      // VTEX às vezes mostra alerta com classes diferentes; deixei mais abrangente
      cy.get('.vtexIdUI-alert, [class*="vtexIdUI-alert"], [role="alert"]', { timeout: 20000 })
        .filter(':visible')
        .first()
        .should('be.visible')
        .invoke('text')
        .should('match', /erro|inválid|falha|incorreto|não foi possível|tente novamente/i)
    })
    ...
    it('Teste 8 — Login com sucesso e acessar relatório de pedidos', () => {
      ...
      // Pós-login: às vezes aparece “Olá, Nome” ou só “Olá”
      cy.contains(/Olá/i, { timeout: 30000 })
        .should('be.visible')

      // “Meus pedidos” pode estar em menu/dropdown.
      // Tentativa 1: link visível direto
      cy.get('body').then(($body) => {
        ...
        if (linkDireto) {
          ...
          // Se não estiver visível, abre o menu de usuário e tenta de novo
